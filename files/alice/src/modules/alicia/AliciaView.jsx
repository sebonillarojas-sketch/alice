...
// ── Panel Equipo · vista read-only + insights de coaching (solo CEO) ──────────
const BRAIN_BASE = import.meta.env.VITE_ALICIA_URL || "http://localhost:3001";

function EquipoPanel({ profiles, currentUserId }) {
  const team = Object.values(profiles).filter(p => p.userId !== currentUserId);
  const [uid, setUid] = useState(team[0]?.userId || null);
  ...
  useEffect(() => {
    if (!uid) return;
    setHistory([]); setInsights(null); setErr(null);
    fetch(`${BRAIN_BASE}/api/history/${uid}?limit=40`).then(r => r.json()).then(d => setHistory(Array.isArray(d) ? d : [])).catch(() => {});
    fetch(`${BRAIN_BASE}/api/insights/${uid}`).then(r => r.json()).then(d => { setInsights(d.report); setInsightsAt(d.updated_at); }).catch(() => {});
  }, [uid]);

  const refresh = async () => {
    setLoadingIns(true); setErr(null);
    try {
      const r = await fetch(`${BRAIN_BASE}/api/insights/${uid}/refresh`, { method: "POST" });
      const d = await r.json();
      if (d.report) { setInsights(d.report); setInsightsAt(d.updated_at); }
      else setErr(d.reason || d.error || "Sin datos suficientes");
    } catch (e) { setErr(e.message); }
    setLoadingIns(false);
  };

  const kicker = { fontSize: 9, color: C.muted, letterSpacing: "0.12em", textTransform: "uppercase", fontWeight: 700 };
  const card = { backgroundColor: C.paper, border: `1px solid ${C.lineSoft}`, borderRadius: 4, padding: 14 };
  const p = profiles[uid];

  const FlagList = ({ title, items, color, icon }) => !items?.length ? null : (
    <div style={{ marginBottom: 14 }}>
      <div style={{ ...kicker, color, marginBottom: 6 }}>{icon} {title}</div>
      {items.map((it, i) => <div key={i} style={{ fontSize: 12, lineHeight: 1.55, color: C.ink, paddingLeft: 10, borderLeft: `2px solid ${color}40`, marginBottom: 5 }}>{it}</div>)}
    </div>
  );
  ...
  return (
    <div style={{ flex: 1, display: "flex", minHeight: 0, overflow: "hidden" }}>
      {/* Lista del equipo */}
      <div style={{ width: 210, borderRight: `1px solid ${C.line}`, backgroundColor: C.paper, overflowY: "auto", padding: 10, flexShrink: 0 }}>
        <div style={{ ...kicker, padding: "4px 6px 8px" }}>Colaboradores</div>
        {team.map(t => (
          <button key={t.userId} onClick={() => setUid(t.userId)}
            style={{ width: "100%", textAlign: "left", padding: "8px 10px", borderRadius: 3, border: "none", cursor: "pointer", backgroundColor: uid === t.userId ? C.bg : "transparent", marginBottom: 2 }}>
            <div style={{ fontSize: 12.5, fontWeight: uid === t.userId ? 700 : 500, color: C.ink }}>{t.name}</div>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0, borderRight: `1px solid ${C.line}` }}>
        <div style={{ padding: "10px 16px", borderBottom: `1px solid ${C.lineSoft}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <span style={{ fontSize: 12, fontWeight: 700, color: C.ink }}>Conversación de {p?.name?.split(" ")[0] || "—"} con Alicia</span>
          <span style={{ fontSize: 9.5, color: C.muted, fontStyle: "italic" }}>👁 solo lectura</span>
        </div>
        <div style={{ flex: 1, overflowY: "auto", padding: 16, display: "flex", flexDirection: "column", gap: 10 }}>
          {history.length === 0 && <div style={{ fontSize: 12, color: C.muted, fontStyle: "italic", textAlign: "center", marginTop: 30 }}>Sin conversaciones registradas en el backend todavía.</div>}
          {history.map((m, i) => (
            <div key={i} style={{ alignSelf: m.role === "user" ? "flex-end" : "flex-start", maxWidth: "80%" }}>
              <div style={{ padding: "8px 12px", borderRadius: 8, fontSize: 12, lineHeight: 1.55, backgroundColor: m.role === "user" ? BAM + "18" : C.paper, border: `1px solid ${m.role === "user" ? BAM + "30" : C.lineSoft}`, color: C.ink }}>
      ...
      <div style={{ width: 330, overflowY: "auto", padding: 14, flexShrink: 0, backgroundColor: C.bg }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
          <div>
            <div style={{ fontSize: 13, fontWeight: 700, color: C.ink }}>Análisis de {p?.name?.split(" ")[0] || "—"}</div>
            {insightsAt && <div style={{ fontSize: 9.5, color: C.muted }}>Actualizado {new Date(insightsAt.replace(" ", "T") + (insightsAt.includes("Z") ? "" : "Z")).toLocaleDateString("es-PE", { day: "numeric", month: "short" })}</div>}
          </div>
          <button onClick={refresh} disabled={loadingIns}
            style={{ padding: "5px 10px", borderRadius: 3, border: "none", backgroundColor: C.ink, color: C.bg, fontSize: 10, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", gap: 5 }}>
            {loadingIns ? <Loader2 size={11} style={{ animation: "spin 1s linear infinite" }} /> : <Sparkles size={11} />}
            {loadingIns ? "Analizando…" : "Analizar"}
          </button>
        </div>
        {err && <div style={{ fontSize: 11, color: "#A85B5B", marginBottom: 10 }}>{err}</div>}
        {!insights && !err && <div style={{ fontSize: 11.5, color: C.muted, fontStyle: "italic" }}>Sin análisis todavía — dale a "Analizar" para que Alicia genere el reporte de coaching.</div>}
        {insights && (
          <div style={card}>
            {insights.resumen && <div style={{ fontSize: 12, lineHeight: 1.6, color: C.ink, marginBottom: 14, fontStyle: "italic" }}>{insights.resumen}</div>}
            <FlagList title="Fortalezas" items={insights.fortalezas} color="#5F8A6A" icon="▲" />
            <FlagList title="Áreas de mejora" items={insights.fallas} color="#C2A45A" icon="◆" />
            <FlagList title="Green flags" items={insights.green_flags} color="#5F8A6A" icon="✓" />
            <FlagList title="Red flags" items={insights.red_flags} color="#A85B5B" icon="⚑" />
            <FlagList title="Recomendaciones para vos" items={insights.recomendaciones} color="#3D52D5" icon="→" />
          </div>
        )}
      </div>
    </div>
...
function AjustesPanel({ currentUserId }) {
  ...
  const loadSkills = useCallback(() => {
    fetch(`${BRAIN_BASE}/api/skills`).then(r => r.json()).then(d => setSkills(Array.isArray(d) ? d : [])).catch(() => {});
  }, []);
  ...
  const saveSkill = async () => {
    ...
    await fetch(`${BRAIN_BASE}/api/skills`, {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ...editing, created_by: currentUserId }),
    }).catch(() => {});
    setEditing(null); loadSkills();
  };

  const deleteSkill = async (id) => {
    await fetch(`${BRAIN_BASE}/api/skills/${id}`, { method: "DELETE" }).catch(() => {});
    loadSkills();
  };

  const kicker = { fontSize: 9, color: C.muted, letterSpacing: "0.12em", textTransform: "uppercase", fontWeight: 700 };
  const card = { backgroundColor: C.paper, border: `1px solid ${C.lineSoft}`, borderRadius: 4, padding: 16 };
  const inputStyle = { width: "100%", padding: "8px 10px", borderRadius: 3, border: `1px solid ${C.line}`, backgroundColor: C.bg, color: C.ink, fontSize: 12, fontFamily: "inherit", outline: "none" };

  return (
    <div style={{ flex: 1, overflowY: "auto", padding: 20 }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(340px, 1fr))", gap: 16, maxWidth: 1100, margin: "0 auto" }}>

        {/* Mi Alicia · fine-tune manual */}
        <div style={card}>
          <div style={{ ...kicker, marginBottom: 4 }}>Mi Alicia · solo para vos</div>
          <div style={{ fontSize: 11.5, color: C.muted, lineHeight: 1.5, marginBottom: 10 }}>
            Instrucciones directas que Alicia sigue con máxima prioridad en TUS conversaciones. Escribí cómo querés que te trate, qué formato usar, qué evitar.
          </div>
        ...
        <div style={card}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
            <div style={kicker}>Skills de Alicia · playbooks enseñables</div>
            <button onClick={() => setEditing({ name: "", description: "", content: "" })}
              style={{ padding: "4px 10px", borderRadius: 3, border: `1px solid ${C.line}`, background: "none", fontSize: 10.5, fontWeight: 600, color: C.inkSoft, cursor: "pointer" }}>
              <Plus size={10} style={{ display: "inline", verticalAlign: "-1px" }} /> Nueva
            </button>
          </div>
          <div style={{ fontSize: 11.5, color: C.muted, lineHeight: 1.5, marginBottom: 10 }}>
            Enseñale oficios: cuando una tarea coincida con una skill, Alicia carga el playbook y lo sigue al pie de la letra.
          </div>
          {skills.length === 0 && !editing && <div style={{ fontSize: 11.5, color: C.muted, fontStyle: "italic" }}>Todavía no le enseñaste ninguna skill.</div>}
          {skills.map(s => (
            <div key={s.id} style={{ padding: "10px 12px", borderRadius: 3, border: `1px solid ${C.lineSoft}`, backgroundColor: C.bg, marginBottom: 8 }}>
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8 }}>
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontSize: 12.5, fontWeight: 700, color: C.ink }}>{s.name}</div>
                  <div style={{ fontSize: 11, color: C.muted, marginTop: 2 }}>{s.description}</div>
                </div>
                <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                  <button onClick={() => setEditing({ id: s.id, name: s.name, description: s.description, content: s.content })} style={{ background: "none", border: "none", cursor: "pointer", color: C.muted, padding: 2 }}><Edit3 size={12} /></button>
                  <button onClick={() => deleteSkill(s.id)} style={{ background: "none", border: "none", cursor: "pointer", color: "#A85B5B", padding: 2 }}><Trash2 size={12} /></button>
                </div>
              </div>
            </div>
          ))}
          {editing && (
            <div style={{ marginTop: 10, padding: 12, borderRadius: 3, border: `1px dashed ${C.line}`, display: "flex", flexDirection: "column", gap: 8 }}>
              <input value={editing.name} onChange={e => setEditing({ ...editing, name: e.target.value })} placeholder="Nombre corto · ej: brief-reunion" style={inputStyle} />
              <input value={editing.description} onChange={e => setEditing({ ...editing, description: e.target.value })} placeholder="Cuándo usarla · ej: Cuando pidan preparar un brief de reunión" style={inputStyle} />
              <textarea value={editing.content} onChange={e => setEditing({ ...editing, content: e.target.value })} rows={7} placeholder={"El playbook, paso a paso:\n1. …\n2. …"} style={{ ...inputStyle, resize: "vertical", lineHeight: 1.6 }} />
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={saveSkill} style={{ padding: "7px 16px", borderRadius: 3, border: "none", backgroundColor: C.ink, color: C.bg, fontSize: 11, fontWeight: 700, cursor: "pointer" }}>Guardar skill</button>
                <button onClick={() => setEditing(null)} style={{ padding: "7px 12px", borderRadius: 3, border: `1px solid ${C.line}`, background: "none", fontSize: 11, color: C.muted, cursor: "pointer" }}>Cancelar</button>
              </div>
            </div>
          )}
        </div>
...
export default function AliciaView({ currentUser, tasks = [], addTask, updateTask, allSpaces = [], knowledgeLinks = [], createEvent }) {
  ...
  // ── Modo Hablar · conversación por voz con el blob ───────────────────────────
  const [aliciaTab, setAliciaTab] = useState("hablar"); // hablar | chat | equipo | ajustes

  const talkOnce = useCallback(() => {
    ...
    r.start();
    setListening(true);
  }, [listening, sending, send, stopListening]);
  ...
  // ── Render: main ─────────────────────────────────────────────────────────────
  const lastAliciaMsg = [...messages].reverse().find(m => m.role !== "user");
  const lastUserMsg = [...messages].reverse().find(m => m.role === "user");
  const TABS = [
    { id: "hablar", label: "● Hablar" },
    { id: "chat", label: "💬 Chat" },
    ...(isAdmin ? [{ id: "equipo", label: "👥 Equipo" }, { id: "ajustes", label: "⚙️ Ajustes" }] : []),
  ];

  return (
    <div style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 60px)", backgroundColor: C.bg, overflow: "hidden" }}>

      {/* ── Tab bar ── */}
      <div style={{ display: "flex", alignItems: "center", gap: 4, padding: "8px 20px", borderBottom: `1px solid ${C.line}`, backgroundColor: C.paper, flexShrink: 0 }}>
        <AliciaAvatar size={24} state={avatarState} />
        <span style={{ fontSize: 13, fontWeight: 700, color: C.ink, marginRight: 14, marginLeft: 6 }}>Alicia</span>
        {TABS.map(t => (
          <button key={t.id} onClick={() => setAliciaTab(t.id)}
            style={{ padding: "5px 14px", borderRadius: 3, border: "none", cursor: "pointer", fontSize: 11, fontWeight: 600, letterSpacing: "0.04em", backgroundColor: aliciaTab === t.id ? C.ink : "transparent", color: aliciaTab === t.id ? C.bg : C.muted, transition: "all 0.15s" }}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      {/* ── Modo Hablar · el blob ── */}
      {aliciaTab === "hablar" && (
        <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 24, padding: 30, overflow: "hidden" }}>
          <button onClick={talkOnce} style={{ background: "none", border: "none", cursor: "pointer", padding: 0, transform: listening ? "scale(1.06)" : "scale(1)", transition: "transform 0.3s" }}
            title={listening ? "Escuchando… tocá para cortar" : "Tocá para hablar"}>
            <AliciaAvatar size={180} state={sending ? "thinking" : isSpeaking ? "speaking" : "idle"} />
          </button>
          <div style={{ textAlign: "center" }}>
            <div style={{ fontSize: 21, fontWeight: 700, color: C.ink, letterSpacing: "-0.01em" }}>
              {listening ? "Te escucho…" : sending ? "Pensando…" : isSpeaking ? "…" : `Hola, ${chatProfile?.name?.split(" ")[0] || ""}`}
            </div>
            <div style={{ fontSize: 12.5, color: C.muted, marginTop: 6 }}>
              {listening ? "Hablá con naturalidad — corto solo cuando termines" : "Tocá el blob y hablá · te respondo con voz"}
            </div>
          </div>
          {(lastUserMsg || lastAliciaMsg) && !listening && (
            <div style={{ maxWidth: 560, width: "100%", display: "flex", flexDirection: "column", gap: 8, maxHeight: "34%", overflowY: "auto" }}>
              {lastUserMsg && <div style={{ alignSelf: "flex-end", padding: "8px 14px", borderRadius: "12px 12px 2px 12px", backgroundColor: BAM, color: "#fff", fontSize: 12.5, maxWidth: "85%" }}>{lastUserMsg.content}</div>}
              {lastAliciaMsg && <div style={{ alignSelf: "flex-start", padding: "10px 14px", borderRadius: "12px 12px 12px 2px", backgroundColor: C.paper, border: `1px solid ${C.lineSoft}`, color: C.ink, fontSize: 12.5, lineHeight: 1.6, maxWidth: "92%" }}>{lastAliciaMsg.content}</div>}
            </div>
          )}
          {!(window.SpeechRecognition || window.webkitSpeechRecognition) && (
            <div style={{ fontSize: 11, color: "#A85B5B" }}>Tu navegador no soporta reconocimiento de voz — usá Chrome o Safari, o el tab Chat.</div>
          )}
        </div>
      )}

      {/* ── Modo Equipo (solo CEO) ── */}
      {aliciaTab === "equipo" && isAdmin && <EquipoPanel profiles={profiles} currentUserId={currentUserId} />}

      {/* ── Modo Ajustes (solo CEO) ── */}
      {aliciaTab === "ajustes" && isAdmin && <AjustesPanel currentUserId={currentUserId} />}
