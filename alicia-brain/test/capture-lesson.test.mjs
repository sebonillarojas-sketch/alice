...
test("capture_lesson flow: corrección humana directa → validated en 1 (no espera 3)", () => {
  const db = db0();
  const { id } = proposeLesson(db, { scope: "agent:alicia", source: "correction", lesson: "Confirmá antes de crear una tarea", risk_level: "L1" });
  const res = runGateOnLesson(db, id, { hardRules: [], minEvidence: 1 });
...
test("capture_lesson flow: si choca con una regla dura → rejected (no pasa)", () => {
  const db = db0();
